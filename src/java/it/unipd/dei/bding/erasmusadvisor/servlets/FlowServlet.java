...
public class FlowServlet extends AbstractDatabaseServlet {
			...
			throws ServletException, IOException
	{
		...
		boolean partEnabled = false;
		boolean interestEnabled = false;
		...
		try {
			...
			if (lu.isStudent())
			{
				...
				if (PartecipazioneDatabase.checkParticipationById(conn, ID, lu.getUser()))
				{
					if (!(ValutazioneFlussoDatabase.checkEvaluation(conn, lu.getUser(), ID)))
					{
						evalEnabled = true;
					}
				}
				// determina se abilitare l'inserimento della partecipazione
				else
				{
					partEnabled = true;
				}
				// determina se abilitare l'inserimento dell'interesse
				if (!(InteresseDatabase.checkInterestById(conn, ID, lu.getUser())))
				{
		...
		if (m == null && results != null) 
		{
			...
			if (lu.isStudent() && partEnabled == true)
			{
				req.setAttribute("partEnabled", "enabled");
			}
			else
			{
				req.setAttribute("partEnabled", "notEnabled");
			}

			if (lu.isStudent() && interestEnabled == true)
			{
				req.setAttribute("interestEnabled", "enabled");
			}
			else
			{
				req.setAttribute("interestEnabled", "notEnabled");
			}

			// determina se abilitare l'eliminazione degli insegnamenti riconosciuti
			if (lu.isFlowResp() && results.getFlusso().getRespFlusso().equals(lu.getUser()))
			{
				req.setAttribute("removeAckEnabled", "enabled");
			}
			else
			{
				req.setAttribute("removeAckEnabled", "notEnabled");
			}
